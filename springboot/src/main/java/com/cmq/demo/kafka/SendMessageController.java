...
public class SendMessageController {
    public final static Logger logger = LoggerFactory.getLogger(SendMessageController.class);
    @Autowired
    ...
    public void sendMeessage(
        ...
        ListenableFuture<SendResult<String, String>> listenableFuture = kafkaTemplate
            .send(topic, message);//发送消息，topic不存在将自动创建新的topic
        listenableFuture.addCallback(//添加成功发送消息的回调和失败的回调
            result -> logger.info("send message to {} success", topic), ex -> logger
                .info("send message to {} failure,error message:{}", topic, ex.getMessage()));
    }
    ...
    public void sendMeessagedefault() {//发送消息到默认的topic
        ...
        kafkaTemplate.send("Topic1","222eeeeeeeee22");
    }
}
